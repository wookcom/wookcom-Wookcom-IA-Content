  ...
  if (isIntro) {
    return (
      <div className="min-h-screen w-full flex items-center justify-center bg-transparent p-4">
        <div className="max-w-2xl w-full bg-brand-surface backdrop-blur-2xl p-6 sm:p-8 rounded-3xl shadow-2xl border border-slate-700/50 text-center animate-fade-in-scale">
          <h1 className="text-4xl sm:text-5xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-white to-brand-secondary mb-4">Empecemos.</h1>
          <p className="text-brand-subtle mb-8 text-base sm:text-lg">
            Primero, dale un nombre a este perfil. Puede ser tu marca, un cliente o un proyecto.
          </p>
          <input
            type="text"
            value={profileName}
            ...
            autoFocus
          />
          <p className="text-brand-subtle mb-10 text-sm sm:text-base">
            Luego, te haré unas preguntas para entrenar a la IA con tu voz, cliente y productos. ¡Es clave para un contenido auténtico!
          </p>
  ...
  return (
    <div className="min-h-screen w-full flex flex-col items-center justify-center bg-transparent p-4 text-brand-text animate-fade-in-scale">
      <div className="w-full max-w-3xl flex-grow flex flex-col justify-center">
        <div className="mb-8">
          <div className="flex justify-between items-center text-xs sm:text-sm font-semibold text-brand-subtle mb-2">
        ...
        <div className="flex-grow flex flex-col justify-center">
            <label htmlFor={currentQuestion.id as string} className="text-2xl sm:text-3xl md:text-4xl font-bold mb-8 text-center text-transparent bg-clip-text bg-gradient-to-r from-white to-slate-400">
            ...
            <textarea
                ...
                className="w-full bg-transparent border-b-2 border-slate-600 focus:border-brand-primary text-lg sm:text-xl md:text-2xl text-center py-3 text-brand-text leading-tight focus:outline-none transition-colors"
                autoFocus
            />
            ) : (
            <input
      ...
      <div className="w-full max-w-3xl py-4 flex flex-col sm:flex-row justify-between items-center gap-4">
        ...
        <div className='flex items-center gap-2'>
            ...
            >
            {overallQuestionIndex === totalQuestions - 1 ? (initialProfile ? 'Guardar' : 'Finalizar') : 'Siguiente'}
            </button>
        </div>
      </div>
    </div>
